         ...
         for file_name in fnames:
            if file_name.endswith(".md"):
                with open(dirpath + "\\" + file_name, "r", encoding="utf-8") as f:
                    request_dict = { "name": file_name[:-3], "content": f.read() }

                document = await add_document(request_dict)
                if document:
                    logger.debug(f"file [{dirpath}{file_name}] named [{document['name']}] added into database]")

                logger.debug(f"file [{dirpath}{file_name}] already exists in the database]")
